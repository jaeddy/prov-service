...
> [!NOTE]
> `NeoModel` is more of a full-fledged OGM (object graph mapper) than `py2neo`, which acts as a driver with some OGM-like features. Meanwhile, `neo4j` provides the core Neo4j driver functionality of `py2neo`, but lacks some of the OGM convenience for creating and/or matching nodes and relationships.
